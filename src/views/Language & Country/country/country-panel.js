...
export const CountryProduct = (props) => {
...
return <View className="profile-slider-component" onClick={viewItem}>
        ...
        <Stack direction="horizontal" className="rating">
            ...
            <Button className="bg-primary get-it-button" text="Get It" />
        </Stack>
...
export default function CountrySlider(props) {
  return <Stack className={`product-slider ${props.className}`}>
    ...
    <Stack direction="horizontal" className="products h-scrollbar">
      {
        props.products && props.products.map((product, idx) => <CountryProduct key={`${props.title}-${idx}`} product={product} discount={props.discount} />)
      }
    </Stack>
  </Stack>
}